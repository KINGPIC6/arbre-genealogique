...
- PASS — le canvas de l'arbre est maintenant navigable au clavier :
  flèches pour déplacer, +/- pour zoomer, 0 pour la vue complète
  (`role="application"`, `tabIndex`, `aria-label` explicite, focus
  visible). Recherche et sélection au clavier (Tab + Entrée)
  fonctionnaient déjà.
